...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("spectrumxMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("spectrumxMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("spectrumxMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("spectrumx-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running spectrumxMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
